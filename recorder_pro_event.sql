CREATE OR REPLACE PROCEDURE `gzdw2024.gz_dim.recorder_pro_event`(run_date DATE, history_day INT64, hitory_retain_day INT64)
begin
...
-------1.dwd_user_event_di
delete gzdw2024.recorder_pro_02_event.dwd_user_event_di
where event_date>=date_add(run_date,interval -history_day day)
;


insert gzdw2024.recorder_pro_02_event.dwd_user_event_di

with  active_info as(
select distinct
event_date
,user_pseudo_id
FROM  `recorder-pro-50451.analytics_250268757.events_*`,unnest(event_params) event_params
...
,case when stream_id='9692329810' then 'fb.ai.avatar.puzzle' when  (app_info.id='' or app_info.id is null) then 'undefined' else app_info.id end AS package_name
...
delete gzdw2024.recorder_pro_01_basic.dwd_user_active_di
where event_date>=date_add(run_date,interval -history_day day)
;

insert gzdw2024.recorder_pro_01_basic.dwd_user_active_di
select 
a.event_date
,a.user_pseudo_id
...
from gzdw2024.recorder_pro_02_event.dwd_user_event_di a 
left join (
select date_add(event_date,interval -1 day) event_day_1
,user_pseudo_id
from gzdw2024.recorder_pro_02_event.dwd_user_event_di
where event_date>=date_add(run_date,interval -history_day day)
...
delete gzdw2024.recorder_pro_02_event.dws_event_profile_di
where event_date>=date_add(run_date,interval -history_day day)
;

insert gzdw2024.recorder_pro_02_event.dws_event_profile_di
with top_version as (
...
from gzdw2024.recorder_pro_01_basic.dwd_user_active_di
...
,b.is_update
from  gzdw2024.recorder_pro_02_event.dwd_user_event_di a 
left join gzdw2024.recorder_pro_01_basic.dwd_user_active_di b 
...
-----4.dwd_user_new_di
delete gzdw2024.recorder_pro_01_basic.dwd_user_new_di
where event_date>=date_add(run_date,interval -hitory_retain_day day)
;

insert gzdw2024.recorder_pro_01_basic.dwd_user_new_di
select a.event_date
...
,a.app_version
,a.last_app_version
from  gzdw2024.recorder_pro_01_basic.dwd_user_active_di a 
left join  gzdw2024.recorder_pro_01_basic.dwd_user_active_di b on a.user_pseudo_id=b.user_pseudo_id 
where a.is_new=1 
...
delete gzdw2024.recorder_pro_01_basic.dws_basic_indicator_profile_di
where event_date>=date_add(run_date,interval -hitory_retain_day day)
;

insert gzdw2024.recorder_pro_01_basic.dws_basic_indicator_profile_di
...
delete gzdw2024.recorder_pro_02_event.dws_event_param_profile_di
where event_date>=date_add(run_date,interval -history_day day)
;

insert gzdw2024.recorder_pro_02_event.dws_event_param_profile_di
select 
event_date
,package_name
,country
...
inner join  gzdw2024.recorder_pro_01_basic.dwd_user_active_di b on a.user_pseudo_id=b.user_pseudo_id and PARSE_DATE('%Y%m%d',a.event_date) =b.event_date
WHERE _TABLE_SUFFIX >=replace(cast(date_add(run_date,interval -history_day day) as string),'-','') 
and a.user_pseudo_id is not null
...
delete gzdw2024.recorder_pro_02_event.dwd_user_event_param_di
...
INSERT
  gzdw2024.recorder_pro_02_event.dwd_user_event_param_di
SELECT
  event_date,
  user_pseudo_id,
  event_name,
...
delete gzdw2024.recorder_pro_01_basic.dwd_user_userid_di
where event_date>=date_add(run_date,interval -history_day day)
;

insert gzdw2024.recorder_pro_01_basic.dwd_user_userid_di
...
-------9.日活与留存统计
delete gzdw2024.recorder_pro_01_basic.dws_user_active_report
where stats_date>=date_add(run_date,interval -history_day day)
;

insert gzdw2024.recorder_pro_01_basic.dws_user_active_report
...
-------10.日活新增与留存recorder_pro_01_basic
...
and package_name in 
               ...
               WHERE stats_date>=date_add(run_date,interval -history_day day)
                and package_name not in ('fb.ai.avatar.puzzle')
...
delete gzdw2024.recorder_pro_02_event.dws_delivery_report
where
  stats_date>=date_add(run_date,interval - history_day day) ;


INSERT
  gzdw2024.recorder_pro_02_event.dws_delivery_report
	SELECT
		stats_date
		,package_name
		,country
	...
	FROM	
		...
		FROM
			...
			FROM
		        ...
		        FROM
		          `gzdw2024.recorder_pro_02_event.dws_event_param_profile_di`
		        WHERE
